...
class CLIFF_COCO_MPII_Dataset_StageI(Dataset):
    ...
    def __init__(self,
        ...
        print(">>>>>>>>cliff stageI coco mpii with_camera_trans:", self.with_camera_trans)
        ...
        print("load cliff by each person:", ann_file)
        print("len self.img_infos: ", len(self.img_infos))
        print("=" * 50)
    ...
    def load_each_person_annotations(self, ann_file):
        ...
        for i in range(len(raw_infos)):
            ...
            for j in range(person_num):
                ...
                kpts2d_smpl_proj = raw_infos[i]['kpts2d_smpl_proj'][j]
                # vertices = raw_infos[i]['vertices'][j]
                ...
                if np.min(global_translation[:, -1]) < 1.0 or np.max(global_translation[:, -1]) > 26.0:
                    continue
    ...
    def prepare_train_img(self, idx):
        ...
        # length = length * 1.2 # comment for 1.0 scale
        ...
        if self.with_seg:
            gt_seg = mmcv.imread(
                osp.join(self.seg_prefix, ann['file_name'].replace(
                    'jpg', 'png')),
                flag='unchanged')
        ...
        if self.with_kpts3d:
            if not self.regress_trans:
                ...
                # print(">>> cliff_coco_mpii_stageI.py gt_poses[i:i+1, -2:, :].shape: ", gt_poses[i:i+1, -2:, :].shape)
