...
__all__ = [
    ...
    'Numpy2Tensor', 'coco17_to_superset', 'DepthMapTransform'
]
...
class DepthMapTransform(object):
    """Preprocess an DepthMap.
    ...
    # def __call__(self, img, scale, flip=False, keep_ratio=True, interpolation='bicubic'):
    def __call__(self, img, scale, flip=False, keep_ratio=True, interpolation='area'):
        if keep_ratio:
            if img.shape[:2] == scale:
                scale_factor = 1.
            else:
                img, scale_factor = mmcv.imrescale(img, scale, return_scale=True, interpolation=interpolation)
        else:
            img, w_scale, h_scale = mmcv.imresize(img, scale, return_scale=True, interpolation=interpolation)
...
class BboxTransform(object):
    ...
    def __init__(self, max_num_gts=None, use_clip_bbox=True):
        self.max_num_gts = max_num_gts
        self.use_clip_bbox = use_clip_bbox

    def __call__(self, bboxes, img_shape, scale_factor, flip=False, translation=(0, 0)):
        gt_bboxes = bboxes * scale_factor
        if flip:
            gt_bboxes = bbox_flip(gt_bboxes, img_shape)
        
        if self.use_clip_bbox == True:
            gt_bboxes[:, 0::2] = np.clip(gt_bboxes[:, 0::2], 0, img_shape[1] - 1)
            gt_bboxes[:, 1::2] = np.clip(gt_bboxes[:, 1::2], 0, img_shape[0] - 1)

        gt_bboxes[:, 0::2] = gt_bboxes[:, 0::2] + translation[0]
        gt_bboxes[:, 1::2] = gt_bboxes[:, 1::2] + translation[1]
