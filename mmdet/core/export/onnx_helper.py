...
def get_k_for_topk(k, size):
    ...
    """
    if not isinstance(k, int) and not isinstance(size, int):
        size = size.to(k.device)
    ret_k = -1
    if k <= 0 or size <= 0:
        return ret_k
...
def add_dummy_nms_for_onnx(boxes,
    ...
    nms_after = get_k_for_topk(nms_after, (num_box * num_class).to(nms_after.device))

    if nms_after > 0:
        _, topk_inds = scores.topk(nms_after)
        batch_inds = torch.arange(batch_size).view(-1, 1).expand_as(topk_inds).to(topk_inds.device)
        # Avoid onnx2tensorrt issue in https://github.com/NVIDIA/TensorRT/issues/1134 # noqa: E501
